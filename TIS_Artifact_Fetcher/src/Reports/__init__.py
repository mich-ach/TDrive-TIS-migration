...
def _create_deviations_sheet(wb, report, header_font_white, header_fill, thin_border,
    ...
    deviation_headers = [
        "Path", "Artifact Name", "Expected Name Format", "Deviation Type", "Uploader",
        "Details", "Expected Path", "Component ID", "TIS Link"
    ]
    ...
    for row_idx, dev in enumerate(report.deviations, 2):
        ...
        ws_deviations.cell(row=row_idx, column=3, value=dev.get('expected_name_format', '')).border = thin_border
        ws_deviations.cell(row=row_idx, column=4, value=dev['deviation_type']).border = thin_border
        ws_deviations.cell(row=row_idx, column=5, value=dev.get('user', 'UNKNOWN')).border = thin_border
        ws_deviations.cell(row=row_idx, column=6, value=dev.get('deviation_details', '')).border = thin_border
        ws_deviations.cell(row=row_idx, column=7, value=dev.get('expected_path_hint', '')).border = thin_border
        ws_deviations.cell(row=row_idx, column=8, value=dev['component_id']).border = thin_border

        tis_link = dev.get('tis_link', '')
        tis_cell = ws_deviations.cell(row=row_idx, column=9, value=tis_link)
        if tis_link:
        ...
        for col in range(1, len(deviation_headers) + 1):
...
def _create_by_component_type_sheet(wb, report, header_font_white, header_fill, thin_border,
    ...
    for comp_type in sorted(deviations_by_component.keys()):
        ...
        for row_idx, dev in enumerate(devs, 2):
            ...
            ws_comp.cell(row=row_idx, column=2, value=dev.get('component_name', '')).border = thin_border
            ws_comp.cell(row=row_idx, column=3, value=dev.get('expected_name_format', '')).border = thin_border
            ws_comp.cell(row=row_idx, column=4, value=dev['deviation_type']).border = thin_border
            ws_comp.cell(row=row_idx, column=5, value=dev.get('user', 'UNKNOWN')).border = thin_border
            ws_comp.cell(row=row_idx, column=6, value=dev.get('deviation_details', '')).border = thin_border
            ws_comp.cell(row=row_idx, column=7, value=dev.get('expected_path_hint', '')).border = thin_border
            ws_comp.cell(row=row_idx, column=8, value=dev['component_id']).border = thin_border

            tis_link = dev.get('tis_link', '')
            tis_cell = ws_comp.cell(row=row_idx, column=9, value=tis_link)
            if tis_link:
                tis_cell.hyperlink = tis_link
                tis_cell.style = "Hyperlink"
            tis_cell.border = thin_border
...
def _create_valid_artifacts_sheet(wb, report, header_font_white, thin_border, valid_fill,
    ...
    valid_headers = ["Path", "Artifact Name", "Expected Name Format", "Uploader", "Component ID", "TIS Link"]
    ...
    for row_idx, artifact in enumerate(report.valid_paths, 2):
        ...
        ws_valid.cell(row=row_idx, column=3, value=artifact.get('expected_name_format', '')).border = thin_border
        ws_valid.cell(row=row_idx, column=4, value=artifact.get('user', 'UNKNOWN')).border = thin_border
        ws_valid.cell(row=row_idx, column=5, value=artifact['component_id']).border = thin_border

        tis_link = artifact.get('tis_link', '')
        tis_cell = ws_valid.cell(row=row_idx, column=6, value=tis_link)
        if tis_link:
            tis_cell.hyperlink = tis_link
            tis_cell.style = "Hyperlink"
        tis_cell.border = thin_border

        for col in range(1, len(valid_headers) + 1):
            ws_valid.cell(row=row_idx, column=col).fill = valid_fill

    for col_idx in range(1, len(valid_headers) + 1):
