...
This workflow maps software lines from a master file (Excel or CSV) to vVeh_LCO artifacts in TIS.
It uses extracted artifact data (JSON) and creates an Excel mapping report.

The TIS data extraction is handled separately by TIS_Artifact_Fetcher.
This workflow expects vVeh_LCO artifact JSON files as input.

The master file can be either:
- Excel (.xlsx) - original master data format
- CSV (.csv) - generated by TIS_Artifact_Fetcher/src/tis_project_lister.py

Usage:
    python -m vVeh_LCO_Mapping <json_file> <master_file>
    python -m vVeh_LCO_Mapping <json_file>
...
def run_mapping_workflow(json_file: Path, master_file: Path) -> bool:
    """
    Run the vVeh_LCO mapping workflow.

    Args:
        json_file: Path to vVeh_LCO artifacts JSON file
        master_file: Path to master file with software lines (.xlsx or .csv)
    ...
    try:
        ...
        base_output_dir, run_dir, excel_copy_path = DirectoryHandler.initialize_directories(master_file)

        logger.info(f"Input JSON: {json_file}")
        logger.info(f"Input master file: {master_file}")
        ...
        # Read master data (Excel or CSV)
        logger.info("")
        logger.info("Step 2: Reading master data and creating mapping")

        input_reader = InputReader()
        excel_handler = ExcelHandler()

        master_data, error = input_reader.read(str(excel_copy_path))
        if error:
            exit_with_error(f"Error reading master file: {error}")

        software_lines = master_data['software_lines']
        project_data = master_data['project_data']

        logger.info(f"  Software lines from master file: {len(software_lines)}")
...
def main():
    ...
    if len(sys.argv) >= 3:
        ...
        master_file = Path(sys.argv[2]).resolve()
    elif len(sys.argv) == 2:
    ...
    if not json_file:
        ...
        logger.info("")
        logger.info("Usage:")
        logger.info("  python -m vVeh_LCO_Mapping <json_file> <master_file>")
        logger.info("  python -m vVeh_LCO_Mapping <json_file>")
        logger.info("  python -m vVeh_LCO_Mapping")
        logger.info("")
        logger.info("Master file can be Excel (.xlsx) or CSV (.csv)")
        logger.info("CSV can be generated with: python -m src.tis_project_lister")
        logger.info("")
    ...
    if not master_file:
        exit_with_error("No master file specified! Provide as argument or set in config.json")

    if not master_file.exists():
        exit_with_error(f"Master file not found: {master_file}")

    logger.info(f"JSON file: {json_file}")
    logger.info(f"Master file: {master_file}")

    success = run_mapping_workflow(json_file, master_file)
